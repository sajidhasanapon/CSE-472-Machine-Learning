...
X_train = pd.read_excel("ratings_train.xlsx", header=None).values  
# X_validate = pd.read_excel("ratings_validate.xlsx", header=None).values
...
for k in [10, 20, 40]:
    for lambd in [0.01, 0.1, 1, 10.0]:
        U, V = train(ratings_train=X_train, n_latent_factors=k, lambd=lambd, n_iter=5)
